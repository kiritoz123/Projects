...
int GetHighScore()
{
    std::ifstream ip("highscore.txt", std::ifstream::in) ;
...
void UpdateHighScore2(std::string& curName, int &score, int& oldHighScore, int type)
{
    ...
    if(type == 1)
    {
        std::ofstream op ("highscore.txt", std::ofstream::out);
        cnt = 0 ;
	...
	if (PlayButton.IsInside(e, COMMON_BUTTON))
	{
		...
		case SDL_MOUSEBUTTONDOWN:
			...
			bool ReadDone = false;
					...
					name = true;
					std::string PlayerName = InputText(*e,Quit_game, gName, gMenuName, gFont, gRenderer, gClick) ;
...
void RenderHighScore(SDL_Renderer* screen, TTF_Font* gFont)
{
    ...
    while(cnt--)
    {
        ...
        gNameList.LoadFromRenderedText(name, gFont, textColorName, screen) ;
        gScoreList.LoadFromRenderedText(HighScore, gFont, textColorName, screen) ;
